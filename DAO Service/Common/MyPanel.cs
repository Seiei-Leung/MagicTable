...
namespace Common
{
    class aa{}
    /*
    public class MyPanel : PanelControl
    {
        ...
        public Color colorCurr = Color.Blue;   //选中的当前外框颜色
        ...
        public MyPanel(BindingSource bsConfig)
            : base()
        {
            selectedControls = new Dictionary<string, Control>();
        ...
        public void UnLock()
        {
            ...
            foreach (Control ctl in this.Controls)
            {
                //if (!ctl.Visible)
                //    continue;
                if (ctl is MyLine && Navigation)
                {
                    RegistrationControlEvent((ctl as MyLine).pictureArrow);
                    RegistrationControlEvent((ctl as MyLine).pictureLine);
                }
                else
                {
                    RegistrationControlEvent(ctl);
                }
                if (ctl is BaseEdit)
                {
        ...
        public void Lock()
        {
            ...
            foreach (Control ctl in this.Controls)
            {
                ...
                if (ctl is MyLine && Navigation)
                {
                    UnregisterControlEvent((ctl as MyLine).pictureArrow);
                    UnregisterControlEvent((ctl as MyLine).pictureLine);
                }
                else
                {
                    UnregisterControlEvent(ctl);
                }
        ...
        public void RegistrationControlEvent(Control ctl)
        {
        ...
        /// </summary>
        /// <param name="ctl"></param>
        public void UnregisterControlEvent(Control ctl)
        {
            ctl.MouseDown -= new MouseEventHandler(Control_MouseDown);
            ctl.MouseLeave -= new EventHandler(Control_MouseLeave);
            ctl.MouseMove -= new MouseEventHandler(Control_MouseMove);
        ...
        protected virtual void LockControl(Control ctl) { }
        protected virtual void UnLockControl(Control ctl) { }
        ...
        /// <param name="e"></param>
        protected virtual void MyPanel_ControlAdded(object sender, ControlEventArgs e)
        {
        ...
        /// <returns></returns>
        protected virtual bool IsSelected(object sender, Point location)
        {
            return true;
        }
        ...
        private void Control_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (!IsSelected(sender, e.Location))
                return;
        ...
        private void ToDataTable()
        {
            ...
            try
            {
                foreach (Control ctl in selectedControls.Values)
                {
                    ...
                    {
                        //if (ctl.Name.Substring(0, ctl.Name.Length - 6).Equals(controlName))
                        if (ctl.Name.Equals(controlName + "_Label"))
                        {
                            row["LabelWidth"] = ctl.Size.Width;
                            row["LabelHeight"] = ctl.Size.Height;
        ...
        /// </summary>
        /// <param name="ctl"></param>
        public DataRow SetCurrRow(Control ctl)
        {
        ...
        public void SetControlCurr(Control ctl)
        {
            ...
            if (!Controls_.ContainsKey(controlName))
                return;
